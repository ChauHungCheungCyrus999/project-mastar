...
function handleExport(apiRef, columns) {
  ...
  for (const key of fields) {
      if (key !== "__check__" && key !== t('action')) { // Skip the "actions" field
        mRow[key] = row[key];
      }
    }
    return mRow;
...
export default function GridExcelExportMenuItem(props) {
  ...
  const filteredColumns = columns.filter(column => column.field !== "actions");

  return (
    <MenuItem
      onClick={() => {
        handleExport(apiRef, filteredColumns);
